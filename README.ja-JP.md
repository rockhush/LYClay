...
`@agent` で別のエージェントを選ぶと、ClawX はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。ワークスペースはオプトインで、ClawX は既定フォルダを自動マウントしません。より強い実行時分離は OpenClaw の sandbox 設定に依存します。入力欄でワークスペースフォルダを選んだチャットセッションは、左サイドバーで該当ワークスペース配下に表示され、それ以外のセッションは従来どおり下段の時系列リストに並びます。
...
ワークスペースに入る前に DingTalk ログインが必要です。起動時のログイン画面には DingTalk の QR 認可フローが埋め込まれ、設定画面でログアウトするとそのログイン画面に戻ります。開発ビルドには既定の企業アプリ設定が含まれています。AppKey、シークレット、コールバックポートを上書きする場合は `LYCLAW_DINGTALK_CLIENT_ID`、`LYCLAW_DINGTALK_CLIENT_SECRET`、`LYCLAW_DINGTALK_CALLBACK_PORT` を使用できます。
