...
### 🔌 MCP-коннекторы
В разделе боковой панели **Коннекторы** настраиваются серверы Model Context Protocol (встроенные сценарии Notion/GitHub и пользовательские MCP). Тот же файл `~/.openclaw/mcp.json` доступен из **Настройки → MCP и коннекторы**, списка **Службы MCP** и **Редактировать MCP JSON**; после сохранения локальный OpenClaw Gateway по возможности перезагружает конфигурацию.
