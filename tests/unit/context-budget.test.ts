...
describe('resolveContextBudget', () => {
  it('uses dynamic budget for 130K context windows', () => {
    ...
    expectCloseTo(budget.compressionTriggerTokens, 76050, 1);
    expect(budget.compressionTriggerTokens).toBeLessThan(150000);
    expect(budget.hardLimitTokens).toBe(Math.floor(budget.maxInputTokens * 0.95));
  });
  ...
  it('scales for 200K context windows', () => {
    ...
    expectCloseTo(budget.compressionTriggerTokens, 117000, 1);
    expect(budget.hardLimitTokens).toBe(Math.floor(budget.maxInputTokens * 0.95));
  });
  ...
  it('stays conservative for 32K context windows', () => {
    ...
    expect(budget.compressionTriggerTokens).toBeGreaterThanOrEqual(14000);
