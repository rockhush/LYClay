...
function getElectronApp() {
  ...
  const fallbackUserData = process.env.CLAWX_USER_DATA_DIR?.trim() || join(homedir(), '.LYClaw');
...
export function getClawXConfigDir(): string {
  return join(homedir(), '.LYClaw');
}

/**
 * Get LYClaw logs directory
 */
