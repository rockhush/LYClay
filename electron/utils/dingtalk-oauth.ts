  ...
  const result = (async (): Promise<DingTalkLoginResult> => {
    if (options?.openExternal !== false) {
      try {
        ...
        throw new Error('无法打开浏览器，请手动访问授权链接', { cause: error });
      }
    }
    ...
    try {
      ...
      if (cancelled) {
        throw new Error('钉钉登录已取消', { cause: error });
      }
      if (error instanceof Error && error.message === 'timeout') {
        throw new Error('钉钉登录超时，请重试', { cause: error });
      }
      throw error;
    }
...
async function getUserAccessToken(params: {
  ...
  const tokenPreview = data.accessToken.substring(0, Math.min(50, data.accessToken.length));
  logger.info(`[DingTalkOAuth] Access token format: length=${data.accessToken.length}, preview=${tokenPreview}...`);
  logger.info(`[DingTalkOAuth] Is JWT format (starts with eyJ): ${data.accessToken.startsWith('eyJ')}`);
