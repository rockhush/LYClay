...
interface SyncProxyOptions {
   ...
   * currently disabled in LYClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync LYClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because LYClaw proxy is disabled and preserve mode is enabled');
      return;
    }
