...
export async function handleProviderRoutes(
  ...
  if (url.pathname === '/api/providers/validate' && req.method === 'POST') {
    ...
    try {
      const body = await parseJsonBody<{ providerId: string; apiKey: string; options?: { baseUrl?: string; apiProtocol?: string; modelId?: string } }>(req);
      ...
      const modelId = body.options?.modelId;
      sendJson(res, 200, await validateApiKeyWithProvider(providerType, body.apiKey, { baseUrl: resolvedBaseUrl, apiProtocol: resolvedProtocol, modelId }));
    } catch (error) {
