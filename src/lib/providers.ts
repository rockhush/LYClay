...
export function resolveProviderModelForSave(
  ...
  // If model ID field is shown and user provided a value, use it
  if (shouldShowProviderModelId(provider, devModeUnlocked)) {
    return trimmedModelId || provider?.defaultModelId || undefined;
  }

  // If model ID field is not shown but provider has a default model, use it
  // This ensures that providers like DeepSeek always have a model configured,
  // even when the model ID field is hidden (e.g., dev mode locked)
