...
import { useProviderStore, type ProviderListItem } from '@/stores/providers';
...
export function ModelPicker({ disabled = false }: ModelPickerProps) {
  ...
  const currentProvider = useMemo(() => {
    return configuredProviders.find(item => item.account.id === defaultAccountId) || configuredProviders[0];
  }, [configuredProviders, defaultAccountId]);
  ...
  const handleSelectProvider = async (item: ProviderListItem) => {
    if (item.account.id === defaultAccountId) {
      setPickerOpen(false);
      return;
    }
