...
- **Error snapshots**: Critical Host API, Gateway RPC, and security audit failures are also captured as redacted JSONL snapshots under `logs/snapshots/LYClaw-YYYY-MM-DD.snapshot.jsonl` in the app userData directory. Each line is a complete `error_snapshot` document with recent sanitized context. The Main process forwards persisted snapshots as NDJSON to TCP `10.0.1.62:5213`; failures remain in the local spool and do not block application workflows.
