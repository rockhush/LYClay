...
lyclaw 是基于全球现象级开源 AI 智能体框架 OpenClaw 进行深度二次开发的企业级 AI 执行引擎，核心定位是为大模型装上 “可落地的手脚”，打通自然语言指令→任务拆解→逻辑推理→工具调用→本地执行→结果反馈的全链路闭环，让 AI 从 “会聊天” 升级为 “能做事”。
本项目在完整保留 OpenClaw 原生核心能力的基础上，针对中文场景、国内生态、企业级安全与运维需求做了深度优化和功能扩展，解决了原生版本在国内环境适配、权限管控、国产化支持、复杂任务稳定性等方面的痛点，可快速部署在 Windows/macOS/Linux/ 国产化操作系统等环境，支持本地私有化部署，全程数据可控，是构建个人 / 企业专属 AI 岗位助理的理想底座。
<p align="center">
...
ClawX requires DingTalk sign-in before entering the workspace. The startup login screen embeds the DingTalk QR authorization flow from the bundled DWS CLI, so the same authorization also creates DWS's own CLI login state. Logging out from Settings returns the app to that login screen. Development builds include a default enterprise app configuration for non-DWS DingTalk API calls; override it with `LYCLAW_DINGTALK_CLIENT_ID`, `LYCLAW_DINGTALK_CLIENT_SECRET`, and `LYCLAW_DINGTALK_CALLBACK_PORT` when needed.

Device access gate: by default ClawX calls `https://lyclawtoken.lingyiitech.com/api/check-token` with the built-in Authorization value; override them with `CLAWX_DEVICE_ACCESS_URL` and `CLAWX_DEVICE_ACCESS_AUTH_TOKEN` only for local tests. On Windows, ClawX runs the bundled `GetDeviceGUID.exe` helper at startup; on macOS, it reads the machine serial number. The main process sends `POST { "token": "<device-token>", "os_type": "windows" | "mac" }` before setup, DingTalk login, or the workspace is shown. A response with `exists: true` allows entry; `exists: false` blocks entry. Device access caching is disabled by default; set `CLAWX_DEVICE_ACCESS_CACHE_TTL_MS` to a positive value only for local tests. If the endpoint is unavailable, startup fails closed unless `CLAWX_DEVICE_ACCESS_FAIL_OPEN=1`. `CLAWX_DEVICE_GUID_EXE_PATH` can point to a local helper exe for testing, `CLAWX_DEVICE_ACCESS_DEVICE_TOKEN` can override the device token, and `CLAWX_DEVICE_ACCESS_OS_TYPE` can override the posted OS type.
...
Remote SSE and streamable HTTP connectors accept `http`, `https`, `ws`, and `wss` URLs, including explicitly authorized HTTP intranet targets. Every protocol still passes through destination policy checks, so private addresses require a domain/IP grant and link-local or metadata targets remain blocked. Provider-required credentials or signatures may stay in the URL query string: network audits omit the query and fragment, while the original URL remains in the local MCP configuration for the runtime; connector cards and authorization summaries mask query values. Prefer request-header authentication when the provider supports it.

Connector cards show only the tools attributable to that MCP server. ClawX uses a server-scoped Gateway catalog when available and otherwise performs MCP `tools/list` discovery directly for remote SSE or streamable HTTP servers; an unscoped OpenClaw tool catalog is never displayed as the connector's inventory. Discovery failures are shown on the card instead of being represented as an unrelated global tool list.
...
For **Custom** providers used with OpenAI-compatible gateways, ClawX writes a 900-second OpenClaw request timeout by default and you can set a custom `User-Agent` in **Settings → AI Providers → Edit Provider** for compatibility-sensitive endpoints.
...
- **Skill Maintenance Boundary**: Agent file tools may create and edit user-managed Skills under `~/.openclaw/skills`. Skill sources distributed with the app, Codex system Skills, and plugin-cache Skills remain read-only dependencies
- **Skill Workshop Global Publishing**: Skill Workshop create and update proposals publish installed Skills to the managed `~/.openclaw/skills/<skill>/` root and return the exact written `targetSkillFile`; Agent workspace Skill folders are not used as the installation destination. Apply, reject, and quarantine requests are resolved through LYClaw instead of expiring unattended. Standard mode shows a one-time confirmation; Trusted and Confirmations Off resolve supported actions without waiting for Renderer, while unknown plugin approvals are denied
...
- **Error snapshots**: Critical Host API, Gateway RPC, and security audit failures, plus runtime soft-failure notices (e.g. `Agent failed before reply:`, `All models failed`, `Agent couldn't generate a response`) and backend-agent stalls (`backendRunStopped`), are also captured as redacted JSONL snapshots under `logs/snapshots/LYClaw-YYYY-MM-DD.snapshot.jsonl` in the app userData directory. Each line is a complete `error_snapshot` document with recent sanitized context. The Main process forwards persisted snapshots as NDJSON to TCP `10.0.1.62:5213`; failures remain in the local spool and do not block application workflows. P1 tool-execution failures (`Write failed`, `Apply Patch failed`, `Exec failed`, etc.) are also spooled and forwarded, but at lower priority than P0 (P0 preempts P1; P1 is batch-scheduled). User-initiated stops and `sessions.abort`/`aborted` states do not generate snapshots.
