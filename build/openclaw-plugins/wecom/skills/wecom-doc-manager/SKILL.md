...
发起智能文档内容导出任务（异步）。传入 docid 和 content_type，返回 task_id。这是异步导出的第一步，需配合 `smartpage_get_export_result` 轮询获取导出结果。
...
4. **创建智能文档**（仅当用户明确要求「智能文档」或「智能主页」时） → 使用 `wecom_mcp` tool 调用 `wecom_mcp call doc smartpage_create '{"title": "标题", "pages": [{"page_title": "子页面", "content_type": 1, "page_filepath": "/path/to/file.md"}]}'`，保存返回的 docid
5. **获取智能文档内容**（URL 含 `/smartpage/`，异步两步）：
